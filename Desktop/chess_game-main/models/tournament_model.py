...
class Tournament:
    ...
    def create_tournament(self, reference, date_debut, date_fin, nombre_tour,
                          description):
        self.reference = reference
        self.date_début = date_debut
        self.date_fin = date_fin
        self.nombre_tour = nombre_tour
    ...
    def __str__(self):
        return self.reference + ' - ' + self.date_début + ' - ' +\
               self.date_fin + ' - ' + self.description
    ...
    def generate_matches(self):
        ...
        if len(self.rounds) == 1:
            for i in range(int(self.nombre_tour)):
                match = Match(players_in_short[i], players_in_short[i +
                              self.nombre_tour])
                print(match)
            ...
            while len(players_in_short) > 1:
                y = 1
                while self.played_against(players_in_short[0],
                                          players_in_short[y]):
                    y += 1
    ...
    def get_tournament(self):
        self.tournaments_list = Tournament().db.search(Query().type ==
                                                       "tournoi")
